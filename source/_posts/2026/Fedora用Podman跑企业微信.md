...
博主用的是Ubuntu，如果跑在Fedora有点问题，SELinux会阻止容器访问宿主机绑定的文件系统。以下操作与博主文章大致相同。
...
执行 `podman logs -f WeCom` 查看后台日志；点击 <http://localhost:8007/> 进入虚拟机桌面，与主机共享的文件夹在桌面，名为【Shared】。你应该注意到了，这个容器其实只是一个安装和控制程序，底层调用的是KVM。
...
开机登录主机后自动启动容器，想暂时停止运行，在终端执行 `podman stop WeCom` 将容器关机。由于我使用的是Fedora Sliverblue，博主后续操作未能在我电脑上实现，基本只是用浏览器访问桌面。这种方案有以下几个缺点：
